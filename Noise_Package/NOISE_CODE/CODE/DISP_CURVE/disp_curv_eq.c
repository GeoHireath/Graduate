...
main(){
	...
	float stlon[nst],stlat[nst];
        char evtnm[10000][6];
        float evtlon[10000],evtlat[10000];
	float prd[80];
	float ave_vel;
	float lat;
	int nstation;
...
// read in event information, same format as station.lst
        if((ff=fopen("event.lst","r"))==NULL){
                fprintf(stderr,"Cannot open event file event.lst\n");
                exit (-1);
        }
        nevt=0;
        while(fscanf(ff,"%s%f%f",evtnm[nevt],&evtlon[nevt],&evtlat[nevt])!=EOF){
                nevt=nevt+1;
        }
        fclose(ff);
        nevt=nevt-1;
...
//now array vel contains the velocity model of all desired periods
	...
	float eps=0.00001;
//loop to calculate all possible event-station pairs
	for (i=0;i<=nevt;i++) {
		for (j=0;j<=nstation;j++) {
		    sprintf(str,"COR_%s_%s.SAC_PRED\0", evtnm[i],stnm[j]);
		    if ((output=fopen(str,"w"))==NULL){
			fprintf(stderr,"cannot create output: %s\n",str);
			exit(-1);
		    }
		    fprintf(output,"  %d %d %d %s %s %f %f %f %f\n",i+1,j+1,nout+1,evtnm[i],stnm[j],evtlat[i],evtlon[i],stlat[j],stlon[j]);
		    lat1=geog_to_geoc(evtlat[i]);
