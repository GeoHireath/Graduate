  ...
  for(ibase = 0; ibase < nfiles; ibase++){
    do{  /* outer do while */
      ...
      if(af[ibase].fname[jbase].onoff==0){
        ...
        if((fp1 = fopen("dostack.sh", "w"))==NULL) {
          fprintf(stderr, "cannot open dostack.sh \n");
          exit(1);
        }
...
//        fprintf(fp1, "#!/bin/csh\n");
	...
	//fprintf(fp1, "mul %d\n", af[ibase].fname[jbase].ndays);
        ...
        mindays=atoi(dump);
        if(div_by > mindays){
          system("sh dostack.sh"); 
	  sprintf(dump, "mv dostack.sh scripts/%d%s.sh", ibase, af[ibase].fname[jbase].name);
	  system(dump);
        }
    } /* end if loop */
