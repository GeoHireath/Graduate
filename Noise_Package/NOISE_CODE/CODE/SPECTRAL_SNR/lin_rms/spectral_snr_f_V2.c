...
/*--------------------------------------------------------------*/
{
  ...
  double maxP= 100.0; 	/* for a 5-100s period band filtering */
  double minP= 3.0  ;
...
//      write_sac("sig.sac",sig0,&SAC_HEADER);
...
//      printf("%f\n",maxT);
      if ((fabs(sum) < 1e-7) ) {
         ...
         noisemax = sqrt(sum/(float)count);
         snr[k]=signalmax/noisemax;      
//         printf("%f %d %d %f %f\n",per[k],count,(int)((maxT+500)/dt),noisemax,signalmax);
      }
