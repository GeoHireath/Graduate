cc this version lists  more parameter for each component of cross-correlation,
cc which is the arrival time of maximun energy.  
...
ccc    infn includes the list of cross-correlations "COR_*._s"
 ...
 100   read(10,*,end = 101) fname
        ...
        write(*,*) fnsnr
        write(*,*) fssnr
        call findsnr(fssnr,per,ssnrout,speakt)
        call findsnr(fnsnr,per,nsnrout,npeakt)
        call findsnr(fpsnr,per,psnrout,ppeakt)
        write(*,*) psnrout,ppeakt,nsnrout,npeakt
     ...
     1  evlo,evla,stlo,stla, dist, az, baz,ppeakt,npeakt,speakt  
       goto 100
 101   continue
 15     format(A20,3x,3f8.2,4f9.2, f8.0, 2f6.0,3f6.0)
       end
...
c   subrounte used to do snr interpolation
        ...
        subroutine findsnr(fname,per,snrout,peakt)
        character *50 fname
        real*4 period(50),snr(50),time(50)
        real*4 per, snrout,peakt
 ...
 11     read(11,*,end=111) period(iper),snr(iper),time(iper)
         iper = iper+1
         goto 11
 111     continue
